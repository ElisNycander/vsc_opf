function [nx, cx, done, rollback, evnts, cb_data, results] = ...
    n32_callback(k, nx, cx, px, done, rollback, evnts, ...
...
if plot_level
    ...
    if k == 0
        ...
        figure(2)
        plot(cxx.lam_hat(1), 180/pi*angle(cxx.V_hat(idx,1)), '-', 'Color', [0.25 0.25 1]);
        hold on;
        xlabel('\lambda');
...
%         end
        if isfield(cb_data.mpc_base,'desc')
            figure(1)
            title(sprintf(['Voltage magnitude\n' cb_data.mpc_base.desc]));
            figure(2)
            title(sprintf(['Angle (deg)\n' cb_data.mpc_base.desc]));
        end
...
%         autumn = colormap('autumn');
        ...
        figure(1)
        hp1 = plot(nxx.lam', abs(nxx.V(idx,:))',  '-');
        grid on;
        figure(2)
        hp2 = plot(nxx.lam', 180/pi*angle(nxx.V(idx,:))', '-');
        grid on;
        ...
        for iii = 1:length(hp1)
           %bus = sscanf((hp(iii).DisplayName),'%*s%d');
           bus = idx_e(iii);
           area = cb_data.mpc_base.bus(cb_data.mpc_base.bus(:,1)==idx(iii),BUS_AREA);
           if area == 1
               hp1(iii).LineStyle = linestyles{1};
               hp2(iii).LineStyle = linestyles{1};
               %hp(iii).Color = winter(i1,:);
               i1 = i1+coloroffset;
           elseif area == 2
               hp1(iii).LineStyle = linestyles{2};
               hp2(iii).LineStyle = linestyles{2};
               %hp(iii).Color = spring(i2,:);
               i2 = i2+coloroffset;
           elseif area == 3
               hp1(iii).LineStyle = linestyles{3};
               hp2(iii).LineStyle = linestyles{3};
               %hp(iii).Color = summer(i3,:);
               i3 = i3+coloroffset;
           else
               hp1(iii).LineStyle = linestyles{4};
               hp2(iii).LineStyle = linestyles{4};
               %hp(iii).Color = autumn(i4,:);
               i4 = i4+coloroffset;
           end
           if ii > size(hsv,1); ii = rem(ii,size(hsv,1)); end
           hp1(iii).Color = hsv(ii,:);
           hp2(iii).Color = hsv(ii,:);
           ii = ii + coloroffset;
        end
        ...
        if nplots > 0
            ...
            legend(hp1, leg);
            legend(hp2, leg);
            % save handle to plots in output
